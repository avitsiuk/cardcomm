...
function decodeTA(byte: number, i: number): any {
    ...
    if (i === 1) {
        ...
        switch (low) {
            case 1: result.Di = 1; break;
            case 2: result.Di = 2; break;
            case 3: result.Di = 4; break;
            case 4: result.Di = 8; break;
            case 5: result.Di = 16; break;
            case 6: result.Di = 32; break;
            case 7: result.Di = 64; break;
            case 8: result.Di = 12; break;
            case 9: result.Di = 20; break;
            default: break;
        }
        switch (high) {
            case 0: result.Fi = 372; result.fMax = 4; break;
            case 1: result.Fi = 372; result.fMax = 5; break;
            case 2: result.Fi = 558; result.fMax = 6; break;
            case 3: result.Fi = 744; result.fMax = 8; break;
            case 4: result.Fi = 1116; result.fMax = 12; break;
            case 5: result.Fi = 1488; result.fMax = 16; break;
            case 6: result.Fi = 1860; result.fMax = 20; break;
            case 9: result.Fi = 512; result.fMax = 5; break;
            case 10: result.Fi = 768; result.fMax = 7.5; break;
            case 11: result.Fi = 1024; result.fMax = 10; break;
            case 12: result.Fi = 1536; result.fMax = 15; break;
            case 13: result.Fi = 2048; result.fMax = 20; break;
            default: break;
        }
        if (result.Fi > 0 && result.Di > 0) result.cyclesPerETU = result.Fi/result.Di;
    } else if (i === 2) {
        result = {
            T: (byte & 0x0F),
    ...
    if (i === 1) {
        ...
        switch (true) {
            case (PI1 > 0):
                ...
                result.PI1 = PI1;
                switch (I) {
                    case 0: result.I = 25; break;
                    case 1: result.I = 50; break;
                    default: break;
                }
                break;
            default: break;
...
export function decodeAtr(atr: TBinData): IAtrInfo {
    ...
    if (inBuffer.byteLength < 2) {
        throw new Error(`ATR length expected to be at least 2 bytes. Received: ${inBuffer.byteLength}`);
    }
    ...
    switch (inBuffer[0]) {
        case 0x3B: result.TS = 'direct'; break;
        case 0x3F: result.TS = 'inverse'; break;
        default: throw new Error('Error decoding ATR: invalid TS byte value');
    }
    ...
    try {
        ...
        throw new Error(`Error decoding ATR: error reading historical bytes: ${error.message}`);
    }
